...
public class InchargeMapsActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMapClickListener {
        ...
        driverLocationsRef.child(String.valueOf(busNumber)).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    updateDriverMarker(busNumber, driverLocation);
                }
            }
        ...
        if (mMap != null) {
            getBusDataFromFirebase(new DataManager.BusDataCallback() {
                ...
                public void onBusDataReceived(Map<Integer, Map<String, LatLng>> busData) {
                    ...
                    if (busMarkers.containsKey(selectedBusNumber)) {
                        updateDriverMarker(selectedBusNumber, busMarkers.get(selectedBusNumber).getPosition());
                    }
                }
    ...
    private void moveToDriverLocation() {
        if (mMap != null) {
            if (busMarkers.containsKey(selectedBusNumber)) {
                LatLng driverLocation = busMarkers.get(selectedBusNumber).getPosition();
    ...
    private void updateDriverMarker(int busNumber, LatLng driverLocation) {
        float bearing = lastBearing; // Set an initial value

        // Update driver's live location on the map and in the database
        if (busMarkers.containsKey(busNumber)) {
            Marker busMarker = busMarkers.get(busNumber);

            Location lastLocation = new Location("last");
            lastLocation.setLatitude(busMarker.getPosition().latitude);
            lastLocation.setLongitude(busMarker.getPosition().longitude);
        ...
        Marker newBusMarker = mMap.addMarker(new MarkerOptions()
        ...
        busMarkers.put(busNumber, newBusMarker);
